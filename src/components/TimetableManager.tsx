...
import { DAYS_OF_WEEK, DEFAULT_TEACHERS, TeacherInfo, CAMPUSES, isSlotMatchingTeacherOrSubject, getEffectiveTimetableForWeek } from "../data/defaultTimetables";
import { 
  Calendar, 
  UploadCloud, 
  Download, 
...
export const TimetableManager: React.FC<TimetableManagerProps> = ({
  ...
  const [selectedTeacher, setSelectedTeacher] = useState<string>(schoolInfo.teacherName || "Cô Tâm");
  ...
  // Filter classes shown in Master view
  const currentCampusInfo = CAMPUSES.find((c) => c.id === selectedCampus);
  const displayedClasses = selectedCampus === "all"
    ? masterTimetable.classes
    : currentCampusInfo?.classes.filter((c) => masterTimetable.classes.includes(c)) || masterTimetable.classes;
    ...
    <div className="space-y-6">
      ...
      {viewMode === "master" && (
        <div className="bg-white border border-black p-6 sm:p-8 space-y-4 shadow-[4px_4px_0px_rgba(0,0,0,0.1)]">
          <div className="border-b border-black pb-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-stone-600 font-serif">
                {isEn ? "Showing allocations for: " : "Hiển thị phân bổ: "}{displayedClasses.join(", ")}
              </p>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-1.5 p-2 bg-stone-50 border border-black text-xs">
            <span className="text-[10px] uppercase font-bold tracking-wider text-black flex items-center gap-1 mr-1">
              <Building2 className="w-3.5 h-3.5" />
              {isEn ? "Campus:" : "Điểm trường:"}
            </span>
            <button
              onClick={() => setSelectedCampus("all")}
              className={`px-2.5 py-1 text-xs font-bold border border-black cursor-pointer transition-colors ${
                selectedCampus === "all" ? "bg-black text-white" : "bg-white text-black hover:bg-stone-100"
              }`}
            >
              {isEn ? "All Campuses (21 Classes)" : "Toàn trường (21 Lớp)"}
            </button>
            {CAMPUSES.map((camp) => (
              <button
                key={camp.id}
                ...
                className={`px-2.5 py-1 text-xs font-bold border border-black cursor-pointer transition-colors ${
                  selectedCampus === camp.id ? "bg-black text-white" : "bg-white text-black hover:bg-stone-100"
                }`}
              >
                {camp.name} ({camp.classes.length} {isEn ? "classes" : "lớp"})
              </button>
            ))}
          </div>

          {/* Campus Official Note Banner if applicable */}
          {currentCampusInfo?.note && (
            <div className="p-3 bg-amber-50 border border-amber-500 text-amber-900 text-xs flex items-start gap-2">
              ...
              <div className="space-y-0.5">
                <span className="font-bold uppercase tracking-wider text-[10px] block text-amber-800">
                  Lưu ý Ban Giám hiệu phân công tại {currentCampusInfo.name}:
                </span>
                <p className="leading-relaxed italic">{currentCampusInfo.note}</p>
              </div>
            </div>
          )}
      ...
      {viewMode === "teacher" && (() => {
        const matchedTeacher = effectiveTeachers.find((t) => t.name === selectedTeacher);
        ...
        const specialistTags = ["hương", "hoàng", "chiên", "quý", "kiệt", "nghĩa", "minh", "tín", "nhung", "xuân", "nghi", "nhi", "nhớ", "pha", "phương", "phượng", "c.hà", "hà", "uyển"];
        ...
        const getTaughtClasses = (slotKey: string) => {
          ...
          effectiveTimetable.classes.forEach((cls) => {
            ...
            if (!val || val === "SHCM" || val === "Nghỉ CM") return;
