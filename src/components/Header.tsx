        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-8 flex flex-wrap items-center justify-between gap-y-2 py-2">
          ...
          <div className="flex items-center flex-wrap gap-2 sm:gap-3 py-1">
            ...
            <div className="flex items-center gap-1">
              ...
              >
                <optgroup label={isEn ? `-- ${effectiveTeachers.filter((t) => t.type === "homeroom").length} HOMEROOM TEACHERS --` : `-- ${effectiveTeachers.filter((t) => t.type === "homeroom").length} GV CHỦ NHIỆM --`}>
                ...
                <optgroup label={isEn ? `-- ${effectiveTeachers.filter((t) => t.type === "specialist").length} SPECIALIST TEACHERS --` : `-- ${effectiveTeachers.filter((t) => t.type === "specialist").length} GV BỘ MÔN & CHUYÊN --`}>
                ...
                <div className="absolute right-0 mt-1.5 w-72 bg-white border-2 border-black shadow-[4px_4px_0px_rgba(0,0,0,1)] z-50 py-1 text-xs">
                  ...
                  {onExportAllTeachersZip && (
                    ...
                    >
                      ...
                      <div>
                        <div className="font-serif font-bold text-black text-xs flex items-center gap-1.5">
                          {isEn ? "7. Export All 18 Teachers (.ZIP)" : "7. Xuất Trọn Bộ 18 GV (.ZIP)"}
                          <span className="text-[9px] bg-black text-white px-1 py-0.2 font-mono">18 GV</span>
                        </div>
