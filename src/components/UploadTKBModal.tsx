...
export const UploadTKBModal: React.FC<UploadTKBModalProps> = ({
  ...
  const handleLoadPreset = (presetName: string) => {
    ...
    if (presetName === "nhonhoalap" || presetName === "nhon_hoa_lap") {
      chosenSchool = "Trường Tiểu Học Nhơn Hòa Lập";
      targetCls = "1A1";
    } else if (presetName === "tanbinh" || presetName === "tanthanh") {
      chosenSchool = "Trường Tiểu Học Nhơn Hòa Lập";
          ...
          {activeMode === "preset" && (
            <div className="space-y-3 font-serif">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <button
                  type="button"
                  onClick={() => handleLoadPreset("nhonhoalap")}
                  className="p-3.5 border-2 border-black bg-emerald-50 hover:bg-emerald-100 text-left transition-all shadow-[2px_2px_0px_rgba(0,0,0,1)] cursor-pointer group"
                >
                  <div className="font-bold text-xs text-black group-hover:underline flex items-center justify-between">
                    <span>TH Nhơn Hòa Lập (Tuần 2)</span>
                    <span className="text-[10px] bg-emerald-700 text-white px-1.5 py-0.2 rounded-none font-sans font-bold">MỚI</span>
                  </div>
                  <div className="text-[11px] text-stone-700 mt-1 font-serif">
                    TKB chính thức Tuần 2 (14/09 - 18/09/2026), 21 lớp - 4 điểm trường (Chính, Hải Hưng, Cây Sao, Bằng Lăng), đồng bộ toàn bộ GVCN &amp; GVBM.
                  </div>
                </button>
